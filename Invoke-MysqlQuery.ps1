...
<# 
  ...
  begin
  {
    ...
    {
      Invoke-WebRequest -UseBasicParsing -Uri "https://dev.mysql.com/get/Downloads/Connector-Net/mysql-connector-net-8.0.19.msi" -OutFile "$MySQLFolder\adomysql.msi";
      Start-Process -FilePath "$MySQLFolder\adomysql.msi" -Wait -ArgumentList '/quiet /l* log.txt';
    }
    ...
    Write-Log -Message "Running Invoke-MySQLQuery with ParameterSet '$($PSCmdlet.ParameterSetName)'.  Performing query '$Query'"
...
'@
  ...
  process
  {
    ...
    {
      Write-Log -Message "Querying ComputerName '$Computer'"
      ...
      #if troubleshooting uncomment below line
      ...
      <# TODO Check if this is needed
          ...
          { 
              ...
              $handler = [System.Data.SqlClient.SqlInfoMessageEventHandler] { Write-Log -Message "$($_)" } 
